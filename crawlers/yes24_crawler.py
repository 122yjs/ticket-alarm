...
def get_yes24_notices():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            logging.info(f"데이터를 더 불러오기 위해 '{more_button_selector}' 버튼을 {click_count}번 클릭합니다.")
            for i in range(click_count):
                try:
                    # '더보기' 버튼이 화면에 보이고, 클릭 가능할 때까지 기다린 후 클릭합니다.
                    page.click(more_button_selector, timeout=5000) # 5초 내에 버튼이 안 보이면 오류 발생
                    logging.info(f"더보기 클릭 {i+1}/{click_count} 완료...")
                    ...
                    logging.info("'더보기' 버튼을 더 이상 찾을 수 없습니다. 데이터 수집을 시작합니다.")
                    break
